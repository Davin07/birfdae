...
4. **Code Coverage:** Aim for >50% code coverage for all modules
...
8. **Verify Project Integrity:**
   - Run the full project build command to ensure all components compile correctly and any necessary artifacts can be generated.
   - Run the complete test suite (unit and integration tests) to ensure no regressions were introduced.
   - Run the project's linting and static analysis tools.
   - **CRITICAL:** Do not proceed to staging or committing until all these checks pass successfully.

9. **Commit Code Changes (Manual):**
   - Stage all code changes related to the task when satisfied.
   - Propose a clear, concise commit message e.g, `feat(ui): Create basic HTML structure for calculator`.
   - The user will perform the commit manually.
...
### Before Committing (Project Integrity Verification)
```bash
# Run all checks: build, all tests, and linting
./gradlew assembleDebug test lint ktlintCheck detekt
```
...
4. Project Integrity Verification passed (Full build, all tests, and linting)
