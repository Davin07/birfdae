...
class ValidationTestRunner {
    ...
    fun `test basic validation functionality`() {
        ...
        assert(invalidResult.errorMessages.size == 4) {
    ...
    fun `test error handler functionality`() {
        ...
        assert(userFriendlyMessage.isNotEmpty()) {
            "Should return user-friendly message"
        }
        assert(!userFriendlyMessage.contains("RuntimeException")) {
            "Should not contain technical details"
        }

        // Test recoverable error detection
        val recoverableError = java.io.IOException("Temporary file error")
        assert(errorHandler.isRecoverableError(recoverableError)) {
            "IOException should be recoverable"
        }

        val nonRecoverableError = OutOfMemoryError("Out of memory")
        assert(!errorHandler.isRecoverableError(nonRecoverableError)) {
            "OutOfMemoryError should not be recoverable"
        }
    ...
    fun `test safe date calculator functionality`() {
        ...
        assert(nextOccurrenceResult.isSuccess) {
            "Normal date calculation should succeed"
        }
        assert(nextOccurrenceResult.getValueOrFallback() == LocalDate.of(2024, 6, 15)) {
            "Should calculate correct next occurrence"
        }
        ...
        assert(leapYearResult.isSuccess) {
            "Leap year calculation should succeed"
        }
        assert(leapYearResult.getValueOrFallback() == LocalDate.of(2023, 2, 28)) {
            "Should use Feb 28 for leap day in non-leap year"
        }
    ...
    fun `test comprehensive validation scenarios`() {
        ...
        val scenarios =
            listOf(
                ...
                // Leap year
                Triple("Dr. Smith", LocalDate.of(2000, 2, 29), true),
                // Max length name
                Triple("a".repeat(100), LocalDate.now().minusYears(1), true),
                // Invalid scenarios
                // Empty name
                Triple("", LocalDate.now().minusYears(25), false),
                // Future date
                Triple("John Doe", LocalDate.now().plusDays(1), false),
                // Name too long
                Triple("a".repeat(101), LocalDate.now().minusYears(25), false),
                // Invalid characters
                Triple("John@Doe", LocalDate.now().minusYears(25), false),
            )
        ...
        scenarios.forEachIndexed { index, (name, birthDate, shouldBeValid) ->
            ...
            if (shouldBeValid) {
                assert(result.isValid) {
                    "Scenario $index should be valid: $name, $birthDate"
                }
            } else {
                assert(result.isInvalid) {
                    "Scenario $index should be invalid: $name, $birthDate"
                }
            }
        }
